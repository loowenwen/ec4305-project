import csv

def read_csv(csvfilename):
    """
    Reads a csv file and returns a list of lists
    containing rows in the csv file and its entries.
    """
    with open(csvfilename, encoding='utf-8-sig') as csvfile:
        return list(csv.reader(csvfile))

def index_filter(category_col_index, desire, filename):
    working_data = read_csv(filename)
    filtered_list = []
    for transac in working_data[1:]:
        if transac[category_col_index] == desire:
            filtered_list.append(transac)
        else:
            pass
    return filtered_list

completed_list = index_filter(1, "3 ROOM", "workingfile_prices.csv")

def aggregate(list_data):
    storage = dict()
    for transac in list_data:
        month = transac[0]
        if month not in storage.keys():
            storage[month] = []
        storage[month].append(float(transac[2]))
    for month, prices in storage.items():
        average = sum(prices)/len(prices)
        storage[month] = average
    return storage

storage_dict = aggregate(completed_list)

with open('filtered_agg_data_3room.csv', 'w') as csv_file:  
    writer = csv.writer(csv_file)
    for key, value in storage_dict.items():
       writer.writerow([key, value])
